...
pub enum DiagnosticKind {
    ...
    /// An Addressing mode the parser reads and this phase does not assemble:
    /// the PC-relative modes and the three special registers.
    UnimplementedAddressingMode {
        /// The Operand as it was written.
        operand: String,
        /// What it is, as a message names it ("a PC-relative operand").
        description: String,
    ...
    pub fn message(&self) -> String {
        match self {
            ...
            DiagnosticKind::UnimplementedAddressingMode {
                ...
                description,
                ..
            } => format!("`{operand}` is {description}, which s68k does not assemble yet"),
...
mod tests {
    ...
    fn every_kind() -> Vec<(DiagnosticKind, &'static str)> {
        vec![
            ...
            (
                DiagnosticKind::UnimplementedAddressingMode {
                    operand: "label(pc)".to_string(),
                    description: "a PC-relative operand".to_string(),
                    advice: Some("write `label` on its own".to_string()),
                },
                "unimplemented_addressing_mode",
            ),
            (
